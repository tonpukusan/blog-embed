(function() {
  ...
  var nextUrl =
    BLOG_URL +
    FEED_PATH +
    "?alt=json&max-results=50&callback=" +
    CALLBACK_NAME;
  ...
  window[CALLBACK_NAME] = function(data) {
    ...
    if (data.feed && data.feed.link) {
      nextLink = data.feed.link.find(function(l) {
        return l.rel === "next";
      });
    }
  ...
  function displayArticles(data) {
    ...
    data.feed.entry.forEach(function(entry) {
      ...
      var hrefObj = entry.link.find(function(l) {
      ...
      html +=
        ...
        '<div class="article-header" id="div_' +
        postdate +
        '">' +
        '<img src="' +
        img_src +
        ...
        '<div class="copy-article-btn" data-copy-html="div_' +
        postdate +
        '">コピー</div>' +
        '<div class="copy-url-btn" data-url="' +
        href +
        '">URLをコピー</div>' +
        ...
        "</div>" +
        '<div class="posthub-btn" data-posthub="' +
        postHubUrl +
        '" style="display:none;">' +
        '<a href="' +
        postHubUrl +
        '" target="_blank">投稿ハブで開く</a>' +
        "</div>" +
        '<div class="blog-embed-btn" data-url="' +
        href +
        '">記事を埋め込む</div>' +
        "</div>" +
        "</div>";
